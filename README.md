...
| `last`    | `last(value)`                                                                                    |
| `mid`     | `(min + max) / 2`                                                                                |
| `sma`     | `avg(value)` per bucket, then a moving window over N buckets in the plugin (`sma:N`, default 5)  |
| `ema`     | `avg(value)` per bucket, then an exponential moving average in the plugin (`ema:α`, default 0.2) |
...
- **QuestDB worker tuning** cuts idle CPU on low-power hosts. QuestDB sizes its worker pools from the host core count and busy-polls before sleeping — sensible on a server, wasteful on a mostly idle boat database, where threads burn CPU waiting for work that never arrives. The plugin pins its worker pools down and quiets QuestDB's per-commit logging, keeping the messages that matter. One caveat worth knowing when upgrading the QuestDB image: an unrecognised setting is ignored silently, so a property renamed between releases looks exactly like one that worked

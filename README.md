...
- **QuestDB worker tuning** cuts idle CPU on low-power hosts. QuestDB sizes its worker pools from the host core count and busy-polls before sleeping — sensible on a server, wasteful on a mostly idle boat database, where threads burn CPU waiting for work that never arrives. The plugin pins each pool it can name to a single thread and lowers the poll threshold, and quiets QuestDB's per-commit INFO logging (suspensions and parse failures still come through). `src/container-env.ts` is the authoritative list
- After a QuestDB image upgrade, confirm the tuning still applies — QuestDB ignores an unrecognised setting silently, so a property renamed between releases looks exactly like one that worked. `SHOW PARAMETERS` reports each setting's resolved value and whether it came from the environment or a default

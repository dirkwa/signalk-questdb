...
- **`eslint.config.cjs` is `.cjs`** because it uses `require`, which `"type": "module"` forbids in `.js`. `.npmignore` excludes config files by exact filename — rename one and a stale entry silently publishes it. Build output (`public/assets/`, `.mf/`) needs entries in `.gitignore`, `.prettierignore` AND `.npmignore`; the three do not inherit from each other.
